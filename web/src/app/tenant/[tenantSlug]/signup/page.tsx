...
import { Sparkles, Mail, Lock, User, Phone, CheckCircle2, ChevronLeft, ArrowRight, Eye, EyeOff, Calendar } from "lucide-react";

export default function TenantCustomerSignup() {
  ...
  const [birthdate, setBirthdate] = useState("1998-10-12");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      document.title = `Sign Up | ${businessName}`;
    }
  }, [businessName]);
  ...
  const accentTextClass = 
    ...
    accentColor === "rose" ? "text-rose-450" :
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (isWhatsAppConnected && !showOtpInput) {
        try {
          ...
          if (!otpRes.ok) {
            setError(otpData.error || "Failed to dispatch verification code.");
            setLoading(false);
            return;
          }

          // Pick template from localStorage to construct logs (for simulation visual dashboard logs)
          const templatesKey = `tenant_whatsapp_templates_${tenantSlug}`;
          const defaultTemplates = [
            { id: "tmpl_1", type: "signup_otp", name: "OTP Standard", text: "Hello {{name}}! Your verification code for {{brand}} is {{otp}}. This code is valid for 5 minutes. Do not share it with anyone." }
          ];
          const templates = JSON.parse(localStorage.getItem(templatesKey) || JSON.stringify(defaultTemplates));
          const otpTemplates = templates.filter((t: any) => t.type === "signup_otp");
          const randomTemplate = otpTemplates[0] || defaultTemplates[0];

          // Write log to localStorage for dashboard visual preview
          ...
          const newLog = {
            ...
            messageText: `Welcome to ${businessName}, ${name}! Please check your WhatsApp for your secure OTP verification code.`,
          ...
          setSuccess(`A secure WhatsApp verification code has been sent to ${phone}. Please enter it to complete signup.`);
        } catch (err) {
      ...
      // If OTP was sent, verify it using secure server-side endpoint
      if (showOtpInput) {
        try {
          ...
          // Verification passed! Proceed with signup
        } catch (err) {
          setError("Verification system currently unreachable. Please try again later.");
          setLoading(false);
          return;
        }
      }
      ...
      try {
        ...
        if (response.ok && data && !data.fallback) {
          // Perform auto-login to set HTTP-only cookie customer_session_${tenantSlug}
          ...
          if (loginRes.ok) {
            ...
            localStorage.setItem(`tenant_user_session_${tenantSlug}`, JSON.stringify(loginData.customer));
            setSuccess("Account created and logged in successfully! Redirecting to your account...");
            ...
            // If auto-login fails, log message but fallback to session cache so they are logged in visually
            localStorage.setItem(`tenant_user_session_${tenantSlug}`, JSON.stringify(data));
            setSuccess("Account created successfully! Redirecting to login...");
            setTimeout(() => {
              window.location.href = getTenantHref(tenantSlug, "/login");
            }, 1500);
          }
        } else {
          setError(data.error || "Failed to create account. Check details or try a different email.");
        }
      } catch (err) {
        setError("Network error. Backend database offline.");
      ...
      <div className="w-full max-w-md relative z-10 space-y-8">
        ...
        <div className="text-center space-y-2.5">
          ...
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-xs opacity-60">Create your customer membership profile</p>
          </div>
        </div>

        {/* Form Card */}
        <div className={`rounded-3xl p-8 border shadow-2xl transition-all duration-300 space-y-6 ${currentTheme.card}`}>
          <h2 className="text-lg font-bold tracking-tight">Register Member</h2>

          {error && (
            <div className="p-3.5 bg-rose-500/10 border border-rose-500/20 text-rose-450 rounded-xl text-xs font-bold leading-relaxed">
              {error}
            </div>
          )}

          {success && (
            <div className="p-3.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-450 rounded-xl text-xs font-bold flex items-center gap-2">
            ...
            {showOtpInput ? (
              <div className="space-y-4 text-left">
                <div className="space-y-1.5">
                  <label className={`block text-[10px] font-bold uppercase tracking-widest ${labelClasses}`}>WhatsApp OTP Code</label>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className={`flex-1 py-3 text-xs uppercase font-extrabold tracking-widest rounded-xl transition-all cursor-pointer border text-center ${
                      activeTemplate === 'MINIMAL' ? 'border-stone-200 text-stone-700 hover:bg-stone-50' :
                      activeTemplate === 'SPA' ? 'border-teal-150 text-teal-700 hover:bg-teal-50' :
                  ...
                  <button
                    type="submit"
                    className={`flex-1 py-3 text-xs uppercase font-extrabold tracking-widest rounded-xl flex items-center justify-center gap-1.5 transition-all active:scale-98 cursor-pointer shadow-lg ${
              ...
              <>
                <div className="space-y-1.5">
                  <label className={`block text-[10px] font-bold uppercase tracking-widest ${labelClasses}`}>Full Name</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
                ...
                <div className="space-y-1.5">
                  <label className={`block text-[10px] font-bold uppercase tracking-widest ${labelClasses}`}>WhatsApp Mobile</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
                ...
                <div className="space-y-1.5">
                  <label className={`block text-[10px] font-bold uppercase tracking-widest ${labelClasses}`}>Birthday (For Retainer Coupons)</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
          ...
          <div className={`pt-5 border-t flex justify-between items-center text-[11px] ${
            ...
            <span className="opacity-55">Already registered?</span>
            <Link
              href={isMounted ? getTenantHref(tenantSlug, `/login${redirectPath ? `?redirect=${redirectPath}` : ""}`) : `/login${redirectPath ? `?redirect=${redirectPath}` : ""}`}
